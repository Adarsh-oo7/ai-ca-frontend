...
export default function ChatPage() {
  ...
  // Dedicated Live Voice Modal State
  ...
  const handleVoiceModalClose = (transcriptItems?: Array<{ sender: 'student' | 'mentor'; text: string }>) => {
    setIsLiveModalOpen(false);
    if (transcriptItems && transcriptItems.length > 0) {
      const formattedMsgs = transcriptItems.map((item, idx) => ({
        id: generateUniqueId(`voice_${item.sender}_${idx}`),
        sender: item.sender,
        text: item.text,
        timestamp: new Date()
      }));
